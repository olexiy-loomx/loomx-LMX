<script>
  ...
  import { CheckSquare, FileUp, MoreHorizontal, Save, Square, Zap } from 'lucide-svelte';
  ...
  function statesAreMixed(states, ids) {
    const values = ids
      .map((id) => states[id])
      .filter(Boolean)
      .map((state) => `${state.color || ''}|${state.effect || ''}|${state.brightness ?? ''}|${state.speed ?? ''}|${state.frequency ?? ''}`);
  ...
  async function applyDraft() {
    ...
    try {
      ...
      addToast(result.ok ? 'Preset values applied' : result.error || 'No connected children acknowledged', result.ok ? 'success' : 'error');
    } finally {
      applying = false;
    }
  }
  ...
  function saveDraft() {
    ...
    editingPresetId = saved.id;
    presetName = saved.name;
  }

  function loadPresetIntoBuilder(preset) {
    const currentIds = new Set($children.map((child) => child.id));
    const targetIds = Array.isArray(preset.target_ids) ? preset.target_ids.filter(Boolean) : [];
    const stateIds = Object.keys(preset.states || {}).filter((id) => currentIds.has(id));
    const nextIds = targetIds.length
      ? targetIds.filter((id) => currentIds.has(id))
      : (stateIds.length ? stateIds : [...currentIds]);
    const state = preset.state || Object.values(preset.states || {})[0] || {};
    const colors = presetColors(preset);

    editingPresetId = preset.id;
    presetName = preset.name || '';
    selectedIds = new Set(nextIds);
    color = state.color || colors[0] || color;
    effect = effects.includes(state.effect) ? state.effect : 'solid';
    brightnessPercent = brightnessToPercent(state.brightness ?? 255);
    speedPercent = controlToPercent(state.speed ?? 128);
    frequencyPercent = controlToPercent(state.frequency ?? 128);
    lookMode = statesAreMixed(preset.states || {}, nextIds) ? 'per_child' : 'same';
    hydrateChildStates(new Set(nextIds), preset.states || {});
    if (preset.output_targets && Object.keys(preset.output_targets).length) {
      outputMode = 'custom';
      selectedOutputsByChild = Object.fromEntries(Object.entries(preset.output_targets)
        .map(([id, slots]) => [id, Array.isArray(slots) ? slots.map(Number).filter(Boolean) : []])
        .filter(([, slots]) => slots.length));
      hydrateOutputSelection(selectedIds);
    } else {
      outputMode = 'all';
      selectedOutputsByChild = {};
    }
  ...
  function rename(id) {
    ...
    if (!item) return;
    renamePreset(id, item.name.endsWith(' Renamed') ? item.name.replace(' Renamed', '') : `${item.name} Renamed`);
    openMenu = null;
  }

  function presetColors(preset) {
    const stateColors = Object.values(preset.states || {}).map((state) => state.color).filter(Boolean);
    return stateColors.length ? stateColors : preset.colors || [];
  }
  ...
  function presetSummary(preset) {
    ...
    const brightnessText = Number.isFinite(brightness) ? `${Math.round((brightness / 255) * 100)}%` : '100%';
    const targetText = targets.length ? `${targets.length} ${targets.length === 1 ? 'child' : 'children'}` : 'All children';
    const outputTargets = preset.output_targets && Object.keys(preset.output_targets).length ? 'custom outputs' : 'all outputs';
    return `${targetText} · ${outputTargets} · ${effectLabel(state.effect || 'solid')} · ${brightnessText}`;
  }
</script>
...
<div class="tab-page">
  ...
  <section class="composer">
    <div class="composer-title">
      <div>
        ...
        <h3>{editingPresetId ? 'Editing preset' : `${selectedCount} selected`}</h3>
      </div>
    ...
    <div class="compose-actions">
      ...
      <Button disabled={selectedCount === 0} on:click={saveDraft}><Save size={16} />{editingPresetId ? 'Save changes' : 'Save as preset'}</Button>
    </div>
  </section>
  ...
  {#if $presets.length}
    <div class="rows">
      {#each $presets as preset (preset.id)}
        <article class="row" class:editing={editingPresetId === preset.id}>
          <button class="row-main" type="button" on:click={() => loadPresetIntoBuilder(preset)}>
            <span class="preset-label">
              <strong>{preset.name}</strong>
              <small>{presetSummary(preset)}</small>
            </span>
            <span class="swatches">
              {#each presetColors(preset).slice(0, 6) as presetColor}
                <i style={`background: ${presetColor}`}></i>
              {/each}
            </span>
          </button>
          <button class="menu-btn" type="button" aria-label={`Actions for ${preset.name}`} on:click={() => openMenu = openMenu === preset.id ? null : preset.id}>
            <MoreHorizontal size={20} />
          </button>
          {#if openMenu === preset.id}
            <div class="menu">
              <button type="button" on:click={() => { applyPreset(preset.id); openMenu = null; }}>Apply</button>
              <button type="button" on:click={() => rename(preset.id)}>Rename</button>
              <button type="button" on:click={() => { addToast('Preset export ready', 'success'); openMenu = null; }}>Export</button>
              <button class="danger" type="button" on:click={() => { deletePreset(preset.id); openMenu = null; }}>Delete</button>
            </div>
          {/if}
        </article>
      {/each}
  ...
  {:else}
    <p class="empty">No presets yet. Select children, set the look, then save it as a preset.</p>
  {/if}
</div>
